...
def get_selected_device_from_gui():
  ...
  # Variable to store the selected device (made global)
  ...
  def on_select(event):
    ...
    # If an item is selected, update the selected_device variable
    if selected_index >= 0:
      selected_device = devices[selected_index]
      print(f"Selected device: {selected_device}")  # Print immediately

  # Function to handle the selected device (modified to print IP)
  def handle_selected_device():
    if selected_device:
        print(f"Selected device IP: {selected_device['ip']}")  # Print only if a device is selected
        spoofing(selected_device['ip'])
    else:
  ...
  # Create a button to activate handle_selected_device
  handle_button = ttk.Button(window, text="Spoof Selected Device", command=handle_selected_device)
  handle_button.pack()
